...
export async function downloadServerFiles(socket, containerName, serverId) {
    try {
        ...
        stream.on('data', (chunk) => {
            ...
            if (text.includes('oauth.accounts.hytale.com') ||
        ...
        stream.on('end', async () => {
            ...
            if (!checkZip.includes('NO_ZIP')) {
                ...
                await docker.execCommand(`unzip -o ${zipPath} -d ${downloadPath}/extract 2>/dev/null || true`, 60000, containerName);
                await docker.execCommand(`find ${downloadPath}/extract -name 'HytaleServer.jar' -exec cp {} /opt/hytale/ \\; 2>/dev/null || true`, 30000, containerName);
                await docker.execCommand(`find ${downloadPath}/extract -name 'Assets.zip' -exec cp {} /opt/hytale/ \\; 2>/dev/null || true`, 30000, containerName);
