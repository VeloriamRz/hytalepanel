...
async function ensureModsSetup(containerName) {
    try {
        await docker.execCommand(`mkdir -p "${MODS_DIR}"`, 5000, containerName);
        const checkResult = await docker.execCommand(`cat "${METADATA_FILE}" 2>/dev/null || echo "NOT_FOUND"`, 30000, containerName);
...
async function loadMods(containerName) {
    try {
...
async function saveMods(modsData, containerName) {
    try {
...
export async function listInstalledMods(containerName) {
    try {
        const result = await loadMods(containerName);
        if (!result.success) {
            return { success: false, error: result.error, mods: [] };
        }
        const lsResult = await docker.execCommand(`ls -la "${MODS_DIR}" 2>/dev/null | grep -E "\\.(jar|disabled|zip)$" || echo ""`, 30000, containerName);
        const filesInDir = [];
...
export async function installMod(fileBuffer, metadata, containerName) {
    try {
        await ensureModsSetup(containerName);
        ...
        const existingIndex = modsData.mods.findIndex((m) => m.projectId === metadata.projectId && m.versionId === metadata.versionId);
        const modEntry = {
        ...
        if (existingIndex >= 0) {
            if (modsData.mods[existingIndex].fileName !== fileName) {
                await docker.execCommand(`rm -f "${MODS_DIR}/${modsData.mods[existingIndex].fileName}" "${MODS_DIR}/${modsData.mods[existingIndex].fileName}.disabled"`, 5000, containerName);
            }
...
export async function uninstallMod(modId, containerName) {
    try {
        const result = await loadMods(containerName);
        if (!result.success) {
        ...
        await docker.execCommand(`rm -f "${MODS_DIR}/${mod.fileName}" "${MODS_DIR}/${mod.fileName}.disabled"`, 5000, containerName);
...
export async function enableMod(modId, containerName) {
    try {
        const result = await loadMods(containerName);
        if (!result.success) {
            return { success: false, error: result.error };
        ...
        const checkResult = await docker.execCommand(`test -f "${disabledPath}" && echo "EXISTS" || echo "NOT_FOUND"`, 30000, containerName);
...
export async function disableMod(modId, containerName) {
    try {
        const result = await loadMods(containerName);
        if (!result.success) {
            return { success: false, error: result.error };
        ...
        const enabledPath = `${MODS_DIR}/${mod.fileName}`;
        const disabledPath = `${MODS_DIR}/${mod.fileName}.disabled`;
        const checkResult = await docker.execCommand(`test -f "${enabledPath}" && echo "EXISTS" || echo "NOT_FOUND"`, 30000, containerName);
        if (checkResult.includes('EXISTS')) {
...
export async function getMod(modId, containerName) {
    try {
        const result = await loadMods(containerName);
        if (!result.success) {
            return { success: false, error: result.error };
...
export async function updateMod(modId, updates, containerName) {
    try {
        const result = await loadMods(containerName);
        if (!result.success) {
            return { success: false, error: result.error };
